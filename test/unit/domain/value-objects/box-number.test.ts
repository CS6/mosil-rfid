...
describe('BoxNumber Value Object', () => {
  describe('Constructor Validation', () => {
    ...
    it('should throw error for null or undefined', () => {
      expect(() => new BoxNumber(null as any)).toThrow('Box number cannot be empty');
      expect(() => new BoxNumber(undefined as any)).toThrow('Box number cannot be empty');
    });

    it('should throw error for empty string', () => {
      expect(() => new BoxNumber('')).toThrow('Box number cannot be empty');
      expect(() => new BoxNumber('   ')).toThrow('Box number must be exactly 13 characters');
    });
    ...
    it('should throw error for wrong length', () => {
      ...
      invalidLengths.forEach(boxNo => {
        expect(() => new BoxNumber(boxNo)).toThrow('Box number must be exactly 13 characters');
      });
    });
    ...
    it('should throw error for invalid format pattern', () => {
      ...
      invalidPatterns.forEach(boxNo => {
        expect(() => new BoxNumber(boxNo)).toThrow('Box number must contain only uppercase letters and numbers');
      });
    });
  });
